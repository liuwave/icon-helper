---

title: Peace (Hand)(和平（手）) ICON转svg、png下载
name: hand-peace
zhTips: 和平（手）,休息,休战
tags: ["hands"]
pre: fas
search: rest,truce
image: https://iconhelper.cn/svg/fontawesome/solid/hand-peace.svg

---

# Peace (Hand)  <small style="font-size: 60%;font-weight: 100">和平（手）</small>


<div class="detail-page">
<p>
<span><span class="badge-success badge">免费图标</span> </span>
<br/>
<span>
ICON库：
<span class="badge-secondary badge">Font Awesome Icon Free</span> 
</span>
<br/>
<span>
CSS名称：
<span class="badge-secondary badge">fa-hand-peace</span> 
</span>
<br/>
<span>
unicode：
<span class="badge-secondary badge">f25b</span> 
<copy-btn content='f25b' btn-title=""></copy-btn>
<copy-btn :content='String.fromCodePoint(parseInt("f25b", 16))' btn-title="复制U"></copy-btn>
</span><br/><span>样式：<span class="badge-light badge">实心(solid)</span></span>
<br/>
<span>
版本：
<span class="badge-secondary badge">5.0.0</span> 
</span><br/><span>标签：<span class="badge-light badge"><router-link to="/tags/hands.html">手</router-link></span></span>
<br/>
<span>图标来源/作者：<span class="badge-light badge">fontawesome</span></span> 
<br/>
<span>别名：<span class="badge-light badge">rest</span><span class="badge-light badge">truce</span></span><br/><span class="zh-detail">中文描述：<span class="badge-primary badge">和平（手）</span><span class="badge-primary badge">休息</span><span class="badge-primary badge">休战</span><span class="help-link"><span>帮助改进</span>(<a href="https://gitee.com/liuwave/icon-helper/edit/master/json/fontawesome/solid/hand-peace.json" target="_blank" rel="noopener noreferrer">gitee</a><a href="https://github.com/liuwave/icon-helper/edit/master/json/fontawesome/solid/hand-peace.json" target="_blank" rel="noopener noreferrer">github</a></span>)</span><br/>
</p>
</div>
<div class="alert alert-dark">
  <i class="fas fa-hand-peace fa-xs"></i>
  <i class="fas fa-hand-peace fa-sm"></i>
  <i class="fas fa-hand-peace fa-lg"></i>
  <i class="fas fa-hand-peace fa-2x"></i>
  <i class="fas fa-hand-peace fa-3x"></i>
  <i class="fas fa-hand-peace fa-5x"></i>
  <i class="fas fa-hand-peace fa-7x"></i>
</div>
<div>
  <p>引入css文件后，可以用<code>&lt;span&gt;</code>包裹，放在页面中。具体如下所示：    
  </p>
  <div class="alert alert-primary" style="font-size: 14px">
    &lt;span class="fas fa-hand-peace" aria-hidden="true"&gt;&lt;/span&gt;
    <copy-btn content='<span class="fas fa-hand-peace" aria-hidden="true"></span>'></copy-btn>
  </div>
  <div class="alert alert-secondary">
    <i class="fas fa-hand-peace"
    style="font-size: 24px"
    aria-hidden="true"></i> fas fa-hand-peace
    <copy-btn content="fas fa-hand-peace" btn-title="复制图标名称"></copy-btn>
  </div>
</div>
<div id="svg" class="svg-wrap">
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M408 216c-22.092 0-40 17.909-40 40h-8v-32c0-22.091-17.908-40-40-40s-40 17.909-40 40v32h-8V48c0-26.51-21.49-48-48-48s-48 21.49-48 48v208h-13.572L92.688 78.449C82.994 53.774 55.134 41.63 30.461 51.324 5.787 61.017-6.356 88.877 3.337 113.551l74.765 190.342-31.09 24.872c-15.381 12.306-19.515 33.978-9.741 51.081l64 112A39.998 39.998 0 0 0 136 512h240c18.562 0 34.686-12.77 38.937-30.838l32-136A39.97 39.97 0 0 0 448 336v-80c0-22.091-17.908-40-40-40z"/></svg>
</div>
<detail full-name='fa-hand-peace'></detail>
<div class="icon-detail__container">
<p>关于“<b>Peace (Hand)</b>”的评论:</p>
</div>
<Vssue title="关于“Peace (Hand)”的评论" />    
<div><p>图标Peace (Hand)来源于Fontawesome Icon Free，可免费使用，更多关于  Fontawesome Icon Free的信息，参见：<a target="_blank" href="https://iconhelper.cn/fontawesome.html">Fontawesome Icon Free</a>
</p></div>

<div style="padding:2rem 0 " class="page-nav"><p class="inner"><span class="prev">←<router-link to="/icon/regular/hand-paper.html">Paper (Hand)</router-link></span> <span class="next"><router-link to="/icon/regular/hand-peace.html">Peace (Hand)</router-link>→</span></p></div>

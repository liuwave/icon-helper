...
[![icon demo](http://cdn.remixicon.com/preview.svg)](https://remix.iconhelper.cn)

- 官网地址： [remixicon.com](https://remixicon.com)
- icon助手主页： [https://remix.iconhelper.cn](https://remix.iconhelper.cn)

## 使用说明

### 基本用法

直接在[https://remix.iconhelper.cn](https://remix.iconhelper.cn)搜索（支持中文搜索）您想要的图标, 在打开的页面中可以选择大小、颜色，然后以 PNG 或者 SVG 格式进行下载，也可以直接复制 SVG 代码。

设计师小伙伴可以直接点击 `复制SVG` 之后在UI设计软件中直接粘贴矢量图形，使用起来像插件一样简单便捷。
...
>
> `复制SVG` 功能目前支持在 Sketch、Figma、Adobe XD、Adobe Illustrator、Affinity Designer、Affinity Photo 软件中直接粘贴矢量图形。
...
> **Note:** 您可以到 [https://remix.iconhelper.cn](https://remix.iconhelper.cn) 查阅图标对应的名称，然后在名称后添加图标风格，`-line` 代表线性风格，`-fill`代表面型风格。
...
> **Note:** 上面示例中 `#` 后面的`ri-admin-fill`可以替换为任何 Remix Icon 图标名称，您可以到[https://remix.iconhelper.cn](https://remix.iconhelper.cn) 查阅图标对应的名称，然后在名称后添加图标风格，`-line` 代表线性风格，`-fill` 代表面型风格。`remixicon.symbol.svg`文件默认是无法预览查看的，只能通过 `<use>` 使用之后查看。
